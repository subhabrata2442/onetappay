      ...
      <div class="row">
        <div class="col-md-12">
          <div class="card">
            ...
            <div class="card-header"> <a href="{{url('merchant_admin/food/addon_items_create')}}" class="btn btn-dark"> <i class="fa fa-plus" aria-hidden="true"></i>Add New</a> </div>
            <!-- /.card-header -->
            <div class="card-body">
              <div class="table-responsive">
                <table id="example1" class="table table-bordered table-striped tableMd">
                  ...
                  @php $i=1;@endphp
                  @foreach($addon_items_list as $row)
                  @php
                  $logo=Helpers::category_image($row->photo);
                  @endphp
                  <tr>
                    <td>{{ $i }}</td>
                    <td><img src="{{ $logo }}" style="width: 50px;"/></td>
                    <td>{{ $row->sub_item_name }}</td>
                    ...
                    <td><a href="{{ route('food.addonItems.edit', $row->sub_item_id )}}" class="btn btn-success btn-sm" data-toggle="tooltip" title="View"><i class="fa fa-edit"></i></a> <a href="javascript:;" class="btn btn-danger btn-sm delete-icon" data-id="{{$row->sub_item_id }}" data-toggle="tooltip" title="Remove"><i class=" fa fa-trash"></i></a></td>
                  </tr>
                  @php $i++;@endphp
                  @endforeach
                    </tbody>
	...
	$(document).on('click','.delete-icon',function(){
        ...
        $("#dialog-confirm").dialog({
            ...
            buttons: {
                "Yes": function() {
                    $(this).dialog("close");
					location.href = "<?=url('/merchant_admin/food/addon_items/delete?id=')?>" + id;
