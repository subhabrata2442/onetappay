...
<div class="content-wrapper"> 
      ...
      <div class="row">
        ...
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item"><a href="{{url('merchant_admin/food/addon_category')}}">Addon Category</a></li>
  ...
  <section class="content">
    <div class="container-fluid"> @include('messages.flash_messages')
      <div class="card card-default"> {{Form::open(['route'=>['food.subcategory.save'],'name'=>'category','id' =>'category-form','files'=>true])}}
        <input type="hidden" name="category_id" value="{{$data->subcat_id ?? ''}}" />
        <div class="card-body">
          <div class="row">
            <div class="col-md-8">
              <div class="form-group">
                <label for="label">AddOn Category Name</label>
                <input type="text" class="form-control" id="category_name" name="category_name" placeholder="Enter Category Name" value="{{$data->subcategory_name ?? ''}}">
              </div>
              <div class="form-group">
                <label for="category">AddOn Category Description</label>
                <textarea class="form-control" id="category_description" name="category_description">{{$data->subcategory_description ?? ''}}</textarea>
              </div>
              <div class="form-group">
                <label for="status">Status</label>
                <select class="form-control" name="status" id="status">
                  <option value="">--Select Status--</option>
                  <option value="active" @php if(isset($data->status)){if($data->status=='active'){ echo 'selected="selected"';}} @endphp> Active </option>
                  <option value="pending" @php if(isset($data->status)){if($data->status=='pending'){ echo 'selected="selected"';}} @endphp echo> InActive </option>
                </select>
              </div>
            </div>
          </div>
